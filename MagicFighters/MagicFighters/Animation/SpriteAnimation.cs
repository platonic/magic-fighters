...
//Last Modified : 9/28/2012    By: Lisandro Martinez
//-----------------------------------------------------------------------------
#endregion
using System;
...
namespace MagicFighters
{
    class SpriteAnimation
    {
        ...
        public void AddAnimation(string Name, List<Rectangle> providedrectangles, float FrameLength, bool flip = false)
        {
            if (providedrectangles == null)
                return;

            faAnimations.Add(Name, new FrameAnimation(providedrectangles,FrameLength, flip));
            iWidth = Width;
            iHeight = Height;
        ...
        public void AddAnimation(string Name, int X, int Y, int Width, int Height, int Frames,
            float FrameLength, bool flip = false)
        {
            faAnimations.Add(Name, new FrameAnimation(X, Y, Width, Height, Frames, FrameLength,flip));
            iWidth = Width;
            iHeight = Height;
            v2Center = new Vector2(iWidth / 2, iHeight / 2);
        }

        public void AddAnimation(string Name, FrameAnimation frames)
        {
            faAnimations.Add(Name, frames);
            iWidth = Width;
            iHeight = Height;
            v2Center = new Vector2(iWidth / 2, iHeight / 2);
        }
        ...
        public void AddAnimation(string Name, int X, int Y, int Width, int Height, int Frames,
           float FrameLength, string NextAnimation, bool flip = false)
        {
            faAnimations.Add(Name, new FrameAnimation(X, Y, Width, Height, Frames, FrameLength, NextAnimation,flip));
            iWidth = Width;
            iHeight = Height;
            v2Center = new Vector2(iWidth / 2, iHeight / 2);
        }
            ...
            if (bAnimating)
            {
                ...
                if (CurrentFrameAnimation.Flip)
                    ...
                    spriteBatch.Draw(t2dTexture, (v2Position + new Vector2(XOffset, YOffset) + v2Center),
                                    ...
                                    fRotation, v2Center, 1f, effect, 0);
            }
